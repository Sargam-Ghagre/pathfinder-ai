...
export function extractTrustedClientIp(headers) {
  ...
  // Use slice from the right to extract only the trusted portion.
  // This prevents attackers from prepending arbitrary IPs.
  // If TRUSTED_PROXY_COUNT = 2, we slice(-3) to get [client, proxy1, proxy2]
  // and return the first element (client IP).
  const trustedChain = ips.slice(-(TRUSTED_PROXY_COUNT + 1));
  const clientIp = trustedChain[0] || ips[0];
