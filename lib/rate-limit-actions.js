...
export async function checkRateLimit(userId, action) {
  ...
  // Atomic operation: increment only if under limit
  ...
  // If result is empty, we hit the limit
  if (!result || result.length === 0) {
    // Check if entry exists or create it
    ...
    if (!existing) {
      await db.aiRateLimit.create({
        data: { userId, action, windowStart, count: 1, expiresAt: resetAt },
      });
    }
